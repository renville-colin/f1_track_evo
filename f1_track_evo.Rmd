...
ad_quali_2021_l %>%
  ggplot(aes(x = lap_start_rownum, y = LapTime5)) +
  geom_point() +
  labs(title = "Abu Dhabi 2021 Qualifying, all laps from raw data",
       subtitle = "Plotting the entire dataset shows we have fast laps and slow laps",
       x = "Laps",
       y = "Lap Time") +
  scale_x_continuous() +
...
# adjustment from regression output (not sure if this is valid)
...
ad_quali_2021_l %>%
  ...
  filter(Driver %in% c("HAM", "BOT", "VER", "PER")) %>%
  # mutate(LapTime5 = if_else(!(is.na(PitOutTime) & is.na(PitInTime)), LapTime4, NULL)) %>%
  mutate(Laptime5_adj = dplyr::if_else(Compound == "MEDIUM", LapTime5 - 0.22979, LapTime5)) %>%
  # mutate(LapTime5 = if_else(!(is.na(PitOutTime) & is.na(PitInTime)), LapTime4, NULL)) %>%
  ggplot(aes(x = coal_LapStartTime, y = Laptime5_adj)) +
  # geom_line(alpha = 0.5) +
  geom_point(aes(color = Compound),
             alpha = 0.65) +
  geom_smooth(method = "gam") +
...
ad_quali_2021_l %>%
  ...
  filter(Driver %in% c("HAM", "BOT", "VER", "PER")) %>%
  # group_by(Team) %>%
  # mutate(scaled_laptime_team = scale(LapTime5)) %>% View()
  # ungroup(Team) %>%
  ...
  geom_boxplot(width = 0.25,
               show.legend = FALSE) +
  labs(title = "Qualifying Lap Times by Tire Compound (Fast Laps only, Red Bull and Merc only)",
       subtitle = "Abu Dhabi 2021",
       x = "Tire Compound",
       y = "Lap Time",
       color = "Tire",
...
lm(LapTime5 ~ Compound, data = rb_merc_reg) %>% summary()

# add any team with medium fast laps
## team difference is almost certainly more significant that compound
# Merc in mediums is probably faster than AT, McLaren, Ferrari in softs

medium_reg <- ad_quali_2021_l %>%
  inner_join(ad_quali_2021_l_q3_drivers, by = c("Driver")) %>%
  # filter(Driver == "VER") %>%
  # filter(!(is.na(PitOutTime) & is.na(PitInTime))) %>%
  filter(hclust == 1) %>%
  filter(Driver %in% c("HAM", "BOT", "VER", "PER", "LEC", "SAI", "TSU", "NOR"))

lm(LapTime5 ~ Compound, data = medium_reg) %>% summary()
...
ad_quali_2021_l %>%
  ...
  mutate(Laptime5_adj = dplyr::if_else(Compound == "MEDIUM", LapTime5 - 0.22979, LapTime5)) %>%
  # mutate(LapTime5 = if_else(!(is.na(PitOutTime) & is.na(PitInTime)), LapTime4, NULL)) %>%
  ggplot(aes(x = as.factor(session_number_coal), y = Laptime5_adj, color = Compound)) +
  # geom_line(alpha = 0.5) +
  # geom_point(aes(color = Compound),
  #            alpha = 0.65) +
  geom_boxplot(width = 0.25) +
...
lm(LapTime5 ~ Compound, data = merc_rb_fast_laps) %>% summary()
...
ad_quali_2021_l %>%
  ...
  # filter(Driver %in% c("HAM", "BOT", "VER", "PER")) %>%
  # mutate(LapTime5 = if_else(!(is.na(PitOutTime) & is.na(PitInTime)), LapTime4, NULL)) %>%
  ggplot(aes(x = Compound, y = LapTime5, color = Compound)) +
  # geom_line(alpha = 0.5) +
  # geom_point(aes(color = Compound),
  #            alpha = 0.65) +
  geom_boxplot(width = 0.25,
               show.legend = FALSE) +
  # geom_smooth(method = "gam") +
  labs(title = "Qualifying Lap Times by Tire Compound (Fast Laps only)",
       subtitle = "Abu Dhabi 2021",
       x = "Tire Compound",
       y = "Lap Time",
       color = "Tire",
...
- research multiple regression with gams with splines for quali session number
- understand how david was "controlling" for variables in dr who analysis
- cumulative count of laps finished prior to the start of a drivers lap, should be good enough

- how to treat aborted laps? (Sainz last fast lap of Q2, Verstappen last fast lap of Q3)
  - detect if a driver significantly increased lap time from previous fast lap? 

- analysis: are there drivers continually performing better or worse than expected?


```{r}

ad_quali_2021_l_fast <- ad_quali_2021_l %>%
  inner_join(ad_quali_2021_l_q3_drivers, by = c("Driver")) %>%
  filter(hclust == 1) %>%
  # manually filtering out aborted laps (make this more formal in future)
  filter(!(Driver == "SAI" & LapNumber == 15)) %>%
  filter(!(Driver == "VER" & LapNumber == 18)) %>%
  # removing non-existant / faux laps
  filter(!(Driver == "HAM" & LapNumber == 19)) %>% # we know Lewis' 19th lap was just him driving to the grid
  filter(!(Driver == "STR" & LapNumber == 7)) # no evidence that STR lap 7 exists on video
  
# remove
gam_test <- mgcv::gam(LapTime5 ~ s(lap_start_rownum, by = Compound) + Compound + Team,
                      data = ad_quali_2021_l_fast, method = "REML")
# remove
gam.check(gam_test)
...
ad_q3_driver_preds_nc <- mgcv::gam(LapTime5 ~ s(lap_start_rownum) + Team,
                                data = ad_quali_2021_l_fast, method = "REML") %>% # summary()
  # plot()
  broom::augment(data = ad_quali_2021_l_fast) %>% 
...
ad_q3_driver_preds_nc %>%
  ggplot(aes(x = lap_start_rownum, y = LapTime5)) +
  geom_point() +
  # geom_smooth(method = "gam") +
  geom_ribbon(aes(ymin = lower, ymax = upper), alpha = 0.2) +
  geom_line(aes(x = lap_start_rownum, y = .fitted), color = "purple") +
  sec_y_lab_convert(start_sec = 80,
                    end_sec = 90,
                    int_sec = 0.5) +
  facet_wrap(~Team,
...
## EXAMPLE WITH INCLUDED DEPENDENCY FOR TIRE COMPOUND

ad_q3_driver_gam <- mgcv::gam(LapTime5 ~ s(lap_start_rownum, by = Compound) + Compound + Team,
                                data = ad_quali_2021_l_fast, method = "REML") # %>% summary()
  # plot()

gam.check(ad_q3_driver_gam)

ad_q3_driver_preds <- broom::augment(ad_q3_driver_gam, data = ad_quali_2021_l_fast) %>% 
  select(DriverNumber, Driver, LapNumber, lap_start_rownum, Compound, Team,
         session_number_coal, LapTime5, .fitted, .se.fit, .resid) %>%# View()
  mutate(lower = .fitted - 1.96 * .se.fit,
         upper = .fitted + 1.96 * .se.fit)
  ...
  # remove
  mgcv::gam(LapTime5 ~ s(lap_start_rownum) + Compound + Team,
                                data = ad_quali_2021_l_fast, method = "REML") %>% summary()
...
ad_q3_driver_preds %>%
 ...
 # faceted by team, all compounds
## this one is now redundant

ad_q3_driver_preds %>%
  ggplot(aes(x = lap_start_rownum, y = .fitted, color = Compound)) +
  geom_point() +
  geom_line() +
  facet_wrap(~Team,
             nrow = 1) +
...
ad_q3_driver_preds %>%
  ggplot(aes(x = LapTime5, y = .fitted, color = Compound)) +
  geom_point() +
  ...
  # facet_wrap(~Team,
  #            nrow = 1) +
  labs(x = "Actual Lap Time",
       y = "Predicted Lap Time") +
  sec_lab_convert(axis = "x",
                  start_sec = 80,
                  end_sec = 90,
                  int_sec = 0.5) +
  sec_lab_convert(axis = "y",
...
# all Q3 teams
# laps 1:316
# soft tire compound only

fit_predict <- expand.grid(lap_start_rownum = 1:316, # only going up until the amount of laps we have recorded in training
                           Team = unique(ad_q3_driver_preds$Team),
                           Compound = "SOFT")

fit_predict_preds <- broom::augment(ad_q3_driver_gam, newdata = fit_predict) %>% 
  # select(DriverNumber, Driver, LapNumber, lap_start_rownum, Compound, Team,
  #        session_number_coal, LapTime5, .fitted, .se.fit, .resid) %>%# View()
  mutate(lower = .fitted - 1.96 * .se.fit,
         upper = .fitted + 1.96 * .se.fit)
...
fit_predict_preds %>%
  ggplot(aes(x = lap_start_rownum, y = .fitted)) +
  # geom_point() +
  geom_point(data = ad_q3_driver_preds[ad_q3_driver_preds$Compound == "SOFT",],
             aes(x = lap_start_rownum, y = LapTime5, color = Driver)) +
  geom_line(data = ad_q3_driver_preds[ad_q3_driver_preds$Compound == "SOFT",],
...
# aggregate lap time difference
## by team, what was the raw and % difference in lap times

fit_predict_prog <- fit_predict_preds %>%
  group_by(Team) %>%
  mutate(lap_start_rownum_desc = row_number(-lap_start_rownum)) %>%
  filter(lap_start_rownum == 1 | lap_start_rownum_desc == 1) %>% 
  mutate(end_fitted = lead(.fitted)) %>%
  filter(!is.na(end_fitted)) %>%
  ungroup() %>%
  mutate(fitted_prog_pct = ((end_fitted - .fitted) / .fitted),
         fitted_prog = (end_fitted - .fitted))


fit_predict_prog %>%
  ggplot(aes(x = fitted_prog, y = fct_reorder(Team, -fitted_prog))) +
  geom_point() +
  theme_main() +
  theme_main_add()
...
#### Bonus: Analyzing Lando and Max's last laps
- telemetry analysis
...
Expand code to analyze all 2021 qualifying sessionss
- tracks that featured the most / least track evolution throughout the season, and why its important for teams to know
- Best qualifying laps over the entire season, when controlling for track evolution and team pace
...
- Compare average speed increases, across all teams, as sessions increase
  - Can include practice, but qualifying would probably be more relevant
