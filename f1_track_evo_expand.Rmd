...
get_quali_reference <- function(season, circuit) {
  ...
  quali_reference_data <- get_driver_data(season = season,
                                          ...
                                          driver = 'ALL',
                                          data_type = 'laps') %>%
    filter(!(raceName == "zandvoort" & Driver == "LAT" & LapNumber == 15)) %>% # problematic lap, getting towed to pits i think
    ...
    mutate(track_status_1 = dplyr::if_else(TrackStatus == "1", 1, 0),
           ...
           # track_status_1_score_l = zoo::rollsum(track_status_1, k = 4, align = "left", fill = NA),
           # track_status_1_score_r = zoo::rollsum(track_status_1, k = 4, align = "right", fill = NA),
           # lag_track_status_1_score_r = zoo::rollsum(lag(track_status_1), k = 4, align = "right", fill = NA),
           lead_track_status_1_score_l = zoo::rollsum(lead(track_status_1), k = 4, align = "left", fill = NA),
           # track_status_1_score = lag_track_status_1_score_r + lead_track_status_1_score_l,
           # lag_track_status_n1_score_r = zoo::rollsum(lag(track_status_n1), k = 4, align = "right", fill = NA),
           lead_track_status_n1_score_l = zoo::rollsum(lead(track_status_n1), k = 4, align = "left", fill = NA)
           # track_status_n1_score = lag_track_status_n1_score_r + lead_track_status_n1_score_l
           ) %>%
    mutate(last_coal_LapStartTime = lag(coal_LapStartTime),
           ...
           session_break_flag_coal2 = ifelse((last_coal_LapStartTime_delta >= break_seconds) & ((!str_detect(TrackStatus, "5"))), 1, 0)
           # session_break_flag_coal2 = ifelse((last_coal_LapStartTime_delta >= break_seconds) & ((!str_detect(last_TrackStatus, "5"))), 1, 0)
           # session_break_flag_coal2 = ifelse((last_coal_LapStartTime_delta >= break_seconds) & ((!str_detect(last_TrackStatus, "5")) |  (!str_detect(TrackStatus, "5"))), 1, 0)
           ) %>%
    fill(session_break_flag_coal, .direction = "up") %>%
    fill(session_break_flag_coal2, .direction = "up") %>%
    mutate(session_number_coal = as.factor(as.character((cumsum(session_break_flag_coal) + 1))),
           session_number_coal2 = as.factor(as.character((cumsum(session_break_flag_coal2) + 1)))) %>%
    ...
    mutate(lead_coal_LapStartTime = lead(coal_LapStartTime),
           delta_time_coal_LapStartTime = (lead_coal_LapStartTime - coal_LapStartTime),
  ...
  quali_reference_agg_coal <- quali_reference_data %>%
    group_by(raceName, session_number_coal2) %>%
...
compile_driver_data <- function(season='2021',
  ...
  if (toupper(session) == 'Q') {
    ...
    lap_data_raw <- get_driver_data(season,
      ...
      fuzzyjoin::fuzzy_join(quali_ref_data,
                        by = c("raceName" = "raceName",
                               "coal_LapStartTime" = "min_coal_LapStartTime",
...
circuits_2021 <- get_weekend_gp(season = '2021') %>%
  dplyr::filter(circuitId %in% c("yas_marina", 
                                 # "americas",
                                 # "red_bull_ring",
                                 "monaco", # going to be a problem, mult. warmup laps plus huge track evo
                                 "imola",
                                 # "rodriguez",
                                 "zandvoort",
                                 # "portimao",
  ...
  select(raceName, round, season, circuitId) # %>%
  # pull()

ad_compile_test_mult <- circuits_2021$circuitId %>%
...
ad_compile_test_mult %>%
  # patchwork some of these below for storytelling
  
  # filter(raceName %in% c("Austrian Grand Prix",
  #                        "Styrian Grand Prix")) %>%
  # filter(raceName == "Monaco Grand Prix") %>%
  # filter(raceName == "Mexico City Grand Prix") %>%
  ...
  facet_wrap(raceName~hclust,
             scales = "free_x",
             nrow = 4) +
  geom_hline(aes(yintercept = ifelse(!is.na(hclust) & raceName == ad_compile_test_mult_cut$raceName, ad_compile_test_mult_cut$hline_val, NA), group = raceName),
                 linetype = "dashed",
                 color = "red") +
...
ad_compile_test_mult %>%
  ...
  facet_wrap(raceName~hclust,
             scales = "free_x",
             nrow = 1) +
  geom_hline(aes(yintercept = ifelse(!is.na(hclust) & raceName == ad_compile_test_mult_cut$raceName, ad_compile_test_mult_cut$hline_val, NA),
                 group = raceName,
...
ad_compile_test_mult %>%
  filter(raceName == "Portuguese Grand Prix") %>%
  ggplot(aes(y = LapTime5, fill = hclust)) +
  geom_histogram(bins = 100,
                 color = "black",
                 show.legend = FALSE) +
...
ad_compile_q3_drivers <- ad_compile_test_mult %>%
  ...
  summarize(max_session = max(as.numeric(as.character(session_number_coal2))),
            max_session2 = max(session_number_coal2),
            .groups = "drop") %>%
...
ad_quali_2021_l_fast_compile <- ad_compile_test_mult %>%
  ...
  inner_join(ad_compile_q3_drivers, by = c("Driver" = "Driver",
                                           "raceName" = "raceName")) %>%
  filter(hclust == 1) %>% # %>% View(title = "test")
...
quali_gam_df <- ad_quali_2021_l_fast_compile %>%
  ...
  group_by(season, raceName, len_Compound) %>%
  nest() %>%
  mutate(gam_model = data %>% map(~gam_quali_model(., compound_len = len_Compound))) %>%
...
# create function to safely unnest and deselect dangerous columns
...
# race_select <- "Qatar Grand Prix"
race_select <- "São Paulo Grand Prix"
...
quali_gam_df %>%
  ...
  geom_line(aes(x = lap_start_rownum, y = .fitted, group = Compound), color = "purple") +
  labs(title = "Lap Time Predictions per Session Progress, Team, Tire Compound",
       subtitle = paste0(race_select, " ", "Qualifying, Drivers who made Q3 only"), 
       x = "Laps Started",
       y = "Lap Time") +
  sec_lab_convert(axis = "y",
                  start_sec = 80,
