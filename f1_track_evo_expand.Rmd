...
get_quali_reference_tbl <- function(tbl) {
  ...
  quali_reference_data <- tbl %>%
    ...
    mutate(track_status_1 = dplyr::if_else(TrackStatus == "1", 1, 0),
           ...
           # track_status_n1 = dplyr::if_else(TrackStatus != "1" | TrackStatus != "2", 1, 0), # attempting to capture red flags, nope
    ...
    mutate(last_coal_LapStartTime = lag(coal_LapStartTime),
           ...
           session_break_flag_coal2 = ifelse((last_coal_LapStartTime_delta >= break_seconds) & ((!(str_detect(TrackStatus, "5")) & lag_track_status_n1_score_r > 2)), 1, 0),
    ...
    fill(session_break_flag_coal, .direction = "up") %>% # remove?
    fill(session_break_flag_coal2, .direction = "up") %>%
    fill(session_break_flag_coal_t, .direction = "up") %>% # remove
    mutate(session_number_coal = as.factor(as.character((cumsum(session_break_flag_coal) + 1))),
           ...
           session_number_coal_t = as.factor(as.character((cumsum(session_break_flag_coal_t) + 1))) # remove
           ) %>%
  ...
  # remove debug logic here
    select(season, raceName, Driver, DriverNumber, LapNumber, lap_start_rownum, PitInTime, PitOutTime,
           ...
           session_break_flag_coal_t, session_number_coal_t, session_ending_flag,
           session_number) %>%
  # remove debug logic here
...
# convert initial script, then deprecate this 
...
get_quali_reference <- function(season, circuit) {
  ...
  quali_reference_data <- get_driver_data(season = season,
                                          ...
                                          driver = 'ALL',
                                          data_type = 'laps') %>%
    filter(!(raceName == "Dutch Grand Prix" & Driver == "LAT" & LapNumber == 15)) %>% # problematic lap, getting towed to pits i think
    filter(!(raceName == "Styrian Grand Prix" & Driver == "BOT" & LapNumber == 15)) %>%
    mutate(Sector1Time_outlap = ifelse(!is.na(PitOutTime), (Sector2SessionTime-PitOutTime-Sector2Time), NA),
           # LapStartTime alone should not be used for this logic
           # use PitOutTime as LapStartTime for outlaps, LapStartTime for non-outlaps
           coal_LapStartTime = coalesce(PitOutTime, LapStartTime)) %>%
...
quali_gam_pred_agg %>%
  ...
  labs(title = "Average Decrease in Lap Times per Qualifying Session",
       ...
       y = "% Decrease in Lap Times (Average)",
       color = "Race") +
