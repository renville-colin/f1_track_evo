...
get_quali_reference_tbl <- function(tbl) {
  ...
  quali_reference_data <- tbl %>%
    ...
    mutate(last_coal_LapStartTime = lag(coal_LapStartTime),
           ...
           # session_break_flag_coal2 = ifelse((last_coal_LapStartTime_delta >= break_seconds) & ((!(str_detect(TrackStatus, "5")) & lag_track_status_n1_score_r > 2)), 1, 0), ## remove
    ...
    # fill(session_break_flag_coal2, .direction = "up") %>% ## remove
    ...
    mutate(session_number_coal = as.factor(as.character((cumsum(session_break_flag_coal) + 1))),
           # session_number_coal2 = as.factor(as.character((cumsum(session_break_flag_coal2) + 1))), ## remove
           session_number_coal_t = as.factor(as.character((cumsum(session_break_flag_coal_t) + 1))) # keep
           ) %>%
  ...
  # remove debug logic here
    select(season, raceName, Driver, DriverNumber, LapNumber, lap_start_rownum, PitInTime, PitOutTime,
           ...
           session_number_coal, #  session_break_flag_coal2,
           # session_number_coal2,
